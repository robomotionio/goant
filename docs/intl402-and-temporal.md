...
| Segmenter | 73 / 79 | the UAX #29 rules this does not implement |
| RelativeTimeFormat | 71 / 80 | per-locale phrasing |
| Intl statics | 60 / 66 | per-locale calendar/collation lists |
| Collator | 58 / 65 | per-locale collation availability |
| DisplayNames | 53 / 57 | CLDR display names |
| PluralRules | 48 / 53 | CLDR plural RANGE rules, which x/text does not carry |
| String, BigInt, Number, Date | 44 / 49 | |

Nine of what is left across all of them is `proto-from-ctor-realm`, which needs
per-function `[[Realm]]` — the same single limitation that leaves `-core` at
42,739 / 42,740.
