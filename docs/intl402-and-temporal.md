...
### A2 — `Intl.NumberFormat`  ▸ **next**

Highest customer value after A0: invoices, amounts, reports. The options bag is
not read at all today; `formatNumber` in `intl_format.go` already does grouping
and the decimal separator per locale, so this is options and parts rather than
arithmetic.

`SetNumberFormatDigitOptions` is already written, in
`builtin_intl_plural.go` — PluralRules needed it first, and the read order
(integer, then fraction, then significant) is observable through getters on the
options bag.
