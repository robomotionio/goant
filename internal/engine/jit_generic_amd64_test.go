		...
		t.Run(expr, func(t *testing.T) {
			...
			for _, x := range ops {
				for _, y := range ops {
					...
					locals[0] = o
					got, gotErr, ok := c.jitRun(rt, fn, locals)
					if !ok {
	...
	for _, body := range []string{
		...
		for _, x := range ops {
			for _, y := range ops {
				...
				locals[0] = o
				got, gotErr, ok := c.jitRun(rt, cl.fn, locals)
				if !ok {
...
func TestJITStringConstantSurvivesCollection(t *testing.T) {
	...
	locals := make([]Value, fn.maxLocals)
	first, _, ok := c.jitRun(rt, fn, locals)
	if !ok {
	...
	rt.Collect()
	second, _, ok := c.jitRun(rt, fn, locals)
	if !ok {
...
func TestJITGenericOperandsSurviveACollection(t *testing.T) {
	...
	locals := make([]Value, fn.maxLocals)
	locals[0] = o
	got, e, ok := c.jitRun(rt, fn, locals)
	if !ok || e != nil {
...
func TestJITGenericOperatorsInALongLoop(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			got, e, ok := c.jitRun(rt, cl.fn, locals)
			if !ok || e != nil {
				t.Fatal("compiled code did not produce a value")
			}
