...
func (r *runner) execute() error {
	...
	for {
		...
		case syntax.Branchmarkjs:
			...
			// Empty match. ECMAScript DISCARDS an optional iteration that consumed
			// nothing and backtracks for a longer one; .NET would end the loop here
			// and keep it. Put the mark back so a retry finds the loop as it was.
			r.stackPush(r.stackPeek())
			break // Backtrack

		case syntax.Branchmarkjs | syntax.Back:
			r.trackPopN(2)
			r.stackPop()
		...
		case syntax.Branchmarkjs | syntax.Back2:
			r.trackPop()
