...
const (
	...
	// Branchmarkjs is Branchmark with ECMAScript's RepeatMatcher rule: an
	// optional iteration that matched the empty string is DISCARDED and the
	// engine backtracks for a longer one, where .NET ends the loop instead.
	Branchmarkjs = 43 // back     jump            branch first for loop, ES nullable rule
...
func opcodeBacktracks(op InstOp) bool {
	...
	switch op {
	case Oneloop, Notoneloop, Setloop, Onelazy, Notonelazy, Setlazy, Lazybranch, Branchmark, Branchmarkjs,
		Lazybranchmark, Nullcount, Setcount, Branchcount, Lazybranchcount, Setmark, Capturemark, Getmark,
		Setjump, Backjump, Forejump, Goto:
		return true
...
func opcodeSize(op InstOp) int {
	...
	case One, Notone, Multi, Ref, Testref, Goto, Nullcount, Setcount, Lazybranch, Branchmark, Branchmarkjs,
		Lazybranchmark, Prune, Set:
		return 2
...
func (c *Code) OpcodeDescription(offset int) string {
	...
	case Goto, Lazybranch, Branchmark, Branchmarkjs, Lazybranchmark, Branchcount, Lazybranchcount:
