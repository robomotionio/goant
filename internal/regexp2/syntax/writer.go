...
func (w *writer) emitFragment(nodetype nodeType, node *regexNode, curIndex int) error {
	...
	case ntLoop | afterChild, ntLazyloop | afterChild:
		...
		} else if lazy == 0 && node.m == 0 {
			// A greedy unbounded loop with no required iterations: every iteration
			// is optional, so ECMAScript's nullable rule applies to all of them.
			w.emit1(Branchmarkjs, w.popInt())
		} else {
