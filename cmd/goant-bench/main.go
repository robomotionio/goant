...
var candidates = []engine{
	...
	// The five interpreters worth standing beside. goja is the other pure-Go
	// engine still in use; otto is the first one, and the far end of the range
	// — ES5, RE2 regular expressions — so it marks where a pure-Go engine used
	// to stop rather than adding another close race; ant is the C engine goant
	// is a port of, so it is the answer to "what did the port cost"; quickjs is
	// the one most embedders actually reach for; duktape is the reference
	// embeddable interpreter. None of the five has a JIT, which is what makes
	// them the honest comparison — node, deno and bun are in the table to show
	// the distance to one.
	{name: "goja", bin: "goja-run"},
	{name: "otto", bin: "otto-run"},
	{name: "ant", bin: "ant"},
